...
src_install() {
	...
	if use nfsboot-server ; then
		...
		cp "${S}/boot/vmlinuz-x86_64-${PV}-gentoo${mPR}-${K_TYPE}" "${D}${nfs_path}/vmlinuz-${K_TYPE}"
		cp "${S}/boot/initramfs-x86_64-${PV}-gentoo${mPR}-${K_TYPE}.img" "${D}${nfs_path}/initramfs-${K_TYPE}"
